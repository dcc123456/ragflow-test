  ...
  async screenshot(name) {
    try {
      if (this.page) {
        ...
        await this.page.screenshot({ path: path.join(this.screenshotsDir, `${name}.png`), fullPage: false });
...
async function runLoginTest(config) {
  ...
  try {
    ...
    await runner.runStep('输入账号密码', async () => {
      ...
      const inputs = await runner.page.$$eval('input', els => els.map(e => ({
        type: e.type, name: e.name, placeholder: e.placeholder, id: e.id
      })));
      runner.log(`找到 ${inputs.length} 个输入框: ${JSON.stringify(inputs)}`);

      // 找到邮箱和密码输入框
      const emailInput = await runner.page.$('input[type="email"], input[type="text"]');
    ...
    await runner.runStep('点击登录按钮', async () => {
      ...
      // 获取所有按钮
      const buttons = await runner.page.$$eval('button', els => els.map(e => ({
        text: e.innerText.trim(), type: e.type
      })));
...
async function runAddLlmTest(config) {
  ...
  try {
    ...
    await runner.runStep('添加模型提供商', async () => {
      // 查找SILICONFLOW文本
      const siliconflow = await runner.page.getByText('SILICONFLOW', { exact: false });
      if (siliconflow) {
